...
LAB_REG=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${API}/v1/lab/assets" \
...
AUTO_ACTIONS=$(curl -s "${API}/v1/responses?incident_id=${INCIDENT_ID}")
...
NOTE_IN_DETAIL=$(curl -s "${API}/v1/incidents/${INCIDENT_ID}" | python3 -c "
import sys, json
...
HTTP_EMPTY=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${API}/v1/incidents/${INCIDENT_ID}/notes" \
  -H "Content-Type: application/json" \
  -d '{"body":"   "}')
...
LOGS=$(curl -s "${API}/v1/responses?incident_id=${INCIDENT_ID}" | python3 -c "
import sys, json
items = json.load(sys.stdin).get('items', [])
...
HTTP_SCOPE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${API}/v1/responses" \
  -H "Content-Type: application/json" \
...
HTTP_T=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${API}/v1/incidents/${INCIDENT_ID}/transitions" \
...
HTTP_NO_REASON=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${API}/v1/incidents/${INCIDENT_ID}/transitions" \
...
HTTP_INVALID=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${API}/v1/incidents/${INCIDENT_ID}/transitions" \
  -H "Content-Type: application/json" -d '{"to_status":"investigating"}')
