...
http_status=$(curl -s -o /dev/null -w "%{http_code}" "${API}/healthz")
...
ct=$(curl -s -D- -o /dev/null --max-time 3 "${API}/v1/stream" 2>/dev/null | grep -i "content-type" | head -1 || true)
...
HB_LOG=$(mktemp)
timeout 25 curl -s -N --max-time 25 "${API}/v1/stream" > "$HB_LOG" 2>/dev/null || true
grep -q ": hb" "$HB_LOG" \
...
STREAM_LOG=$(mktemp)
curl -s -N "${API}/v1/stream" > "$STREAM_LOG" &
STREAM_PID=$!
sleep 1
...
curl -s -X POST "${API}/v1/events/raw" \
...
FILTER_LOG=$(mktemp)
curl -s -N "${API}/v1/stream?topics=actions" > "$FILTER_LOG" &
FILTER_PID=$!
sleep 0.5
...
bad_status=$(curl -s -o /dev/null -w "%{http_code}" "${API}/v1/stream?topics=not_a_topic")
[ "$bad_status" = "400" ] \
...
LOG1=$(mktemp)
LOG2=$(mktemp)
curl -s -N "${API}/v1/stream?topics=detections" > "$LOG1" &
PID1=$!
curl -s -N "${API}/v1/stream?topics=detections" > "$LOG2" &
PID2=$!
sleep 0.5
