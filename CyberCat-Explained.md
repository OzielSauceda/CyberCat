...
25. **Heavy-hitter scenarios + operator drills + merge/split (Phase 20, tag `v1.0`)** — the first phase since 18 to add product surface, and the biggest single jump in analyst-facing capability. **Workstream A — five Linux choreographed scenarios** under `labs/simulator/scenarios/`: `lateral_movement_chain` (5× SSH brute-force on alice → suspicious-IP login → 2-hop pivot through host-1→host-2→host-3 with `curl|sh` persistence; produces an `identity_compromise` incident in current platform state), `crypto_mining_payload` (cryptojacking — `bash → curl xmrig → chmod +x → mining process → pool beacon` to a pre-seeded blocked IP), `webshell_drop` (inbound HTTP → `.php` upload to `/var/www/html/` → `apache2 → sh → id/cat/wget` recon chain), `ransomware_staging` (Linux-flavored, **STAGING ONLY** per CLAUDE.md §8: `find → tar → 30-file .encrypted creation burst over 60s → rm -rf` — no real encryption, no real file writes), `cloud_token_theft_lite` (read `~/.aws/credentials` → curl POST exfil → clean cloud login from new IP). Each ships a `.jsonl` regression fixture under `labs/fixtures/scenario/` and a manifest entry asserting `must_fire`/`must_not_fire`; the detection-as-code suite grew from 8 to 13 scenarios. **Workstream B — operator drills:** `labs/drills/run.sh` orchestrator + 5 markdown walk-throughs that pause at decision points and verify the operator's response (status transitions, action proposals) via the API. CLI-only; frontend training-mode UI explicitly Phase 24+. **Workstream C — merge/split incidents:** schema migration 0009 adds `incidents.parent_incident_id` (self-FK) + a `'merged'` value in `incident_status`; correlator-layer `merge_incidents()` and `split_incident()` in `backend/app/correlation/` use Postgres advisory locks to serialize concurrent operations; two new analyst-only API routes (`POST /v1/incidents/{id}/merge-into` and `/split`) with full error mapping (422/404/409); frontend `MergeModal.tsx` + `SplitButton.tsx` built per the dossier-token aesthetic (cyber-orange "destructive" stamps, candidate-list filter, before/after preview, reason field with 500-char limit); `IncidentTimelineViz.tsx` accepts a `splitMode` prop and renders cyber-orange checkboxes on event chips. ADR-0015 documents the durable design (nullable parent FK over junction table, splits don't set parent FK, downgrade asymmetry). **Workstream D — smoke + docs + tag:** `labs/smoke_test_phase20.sh` (9/9 green: 5 scenarios + merge/split route probes + a real split that moved an event off A1's incident); this section in `CyberCat-Explained.md`; `docs/phase-20-summary.md` with the **Detection gaps** section as the Phase 21 + Phase 22 input list (4 detector candidates: Linux process-chain LotL, blocked-observable correlator promotion, identity-baseline for clean cred theft, file-creation rate burst — each with named scenario evidence + code path); `v1.0` tagged at the close-out commit. **Verification scorecard:** 257/257 backend tests, 15/15 detection-as-code, 9/9 smoke, frontend typecheck clean + image rebuilt + 200 on UI. **Two bugs surfaced + fixed during build/release:** (1) `resolve_actor_id()` is `async` and takes `db` as second arg; the new merge/split routes initially called it without `await db` — caught by integration tests pre-push. (2) Post-push CI tripped on 4 ruff lint errors in the new merge/split code (2× `B904` missing `from exc` on re-raise, 2× `F401` unused imports left over from earlier drafts); fixed in `11778e3` and CI re-ran green. Memory entry codified to run `ruff check app/` locally before pushing backend changes — backend dev image doesn't ship ruff (CI installs it via `[dev]` extras), so a session that only runs pytest can pass-then-fail-CI.

**What's pending:**

- **Phase 19.5 (✅ fully verified locally 2026-05-04):** all six scenarios green + regression-injection sanity check passed both directions. Optional follow-ups deferred to operator preference: (a) `chaos-redis.yml` workflow refactor to source `lib/evaluate.sh` if cross-platform Linux verification of A1 is wanted (current inline eval is the original Phase-19 version that failed three times on 2026-05-02); (b) trigger the five non-redis `chaos-*.yml` workflows from the Actions tab — they're likely to pass on `ubuntu-latest` since the same scripts pass locally. Plan: `docs/phase-19.5-plan.md`.
- **Phase 21:** Caldera adversary emulation + coverage scorecard. Now has a *measured baseline* from Phase 20 (the gap list in `docs/phase-20-summary.md`). Also the natural home for multi-worker uvicorn if 1000/s sustained becomes a real requirement.
